...
from ..serializers import StudentSubjectsScoreSerializer, StudentSerializer
from operator import mod

class StudentSubjectsScoreAPIView(APIView):
    def get(request, *args, **kwargs):
        studentSubjectScore = StudentSubjectsScore.objects.all()
        serializer = StudentSubjectsScoreSerializer(studentSubjectScore, many=True)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        student = Personnel.objects.all().filter(pk=student_id)
        student_serializer = StudentSerializer(student)

        studentSubjectScore = StudentSubjectsScore.objects.all().filter(student=student_id)
        studentSubjectScoreSerializer = StudentSubjectsScoreSerializer(studentSubjectScore, many=True)

        average_point = sum(subjectScore['score'] for subjectScore in studentSubjectScoreSerializer.data) / sum(subjectScore['credit'] for subjectScore in studentSubjectScoreSerializer.data)
        
        # data = {
        #     student_serializer.data, 
        #     "subject_detail": studentSubjectScoreSerializer.data
        # }
        return Response(studentSubjectScoreSerializer.data, status=status.HTTP_200_OK)
